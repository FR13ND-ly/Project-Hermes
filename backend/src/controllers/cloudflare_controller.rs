...
/// credentials (one per project that had a token) and link the project to it. Runs
/// once at boot; idempotent (skips projects that already have a credential id).
pub async fn reconcile_cloudflare_credentials(pool: &sqlx::PgPool) {
    let rows = match sqlx::query!(
        "SELECT id, workspace_id, name, cloudflare_api_token, cloudflare_zone_id, base_domain
         FROM projects
         WHERE cloudflare_credential_id IS NULL
           AND cloudflare_api_token IS NOT NULL AND btrim(cloudflare_api_token) <> ''"
    )
    .fetch_all(pool)
    .await
    {
    ...
    for p in rows {
        let token = p.cloudflare_api_token.unwrap_or_default();
        let token = token.trim();
        if token.is_empty() {
            continue;
        }
        let zone_id = p.cloudflare_zone_id.unwrap_or_default();
        let Ok((enc, nonce)) = crypto::encrypt_env_value(token) else { continue; };
        let cred_id = Uuid::new_v4();
        let label = format!("{} Cloudflare", p.name);
        let inserted = sqlx::query!(
            "INSERT INTO cloudflare_credentials (id, workspace_id, label, encrypted_token, nonce, zone_id, base_domain, created_by)
             SELECT $1, $2, $3, $4, $5, $6, $7, created_by FROM projects WHERE id = $8",
            cred_id, p.workspace_id, label, enc, nonce, zone_id, p.base_domain, p.id
        )
        .execute(pool)
        .await;
        if inserted.is_ok() {
            let _ = sqlx::query!(
                "UPDATE projects SET cloudflare_credential_id = $1 WHERE id = $2",
                cred_id, p.id
            )
            .execute(pool)
            .await;
